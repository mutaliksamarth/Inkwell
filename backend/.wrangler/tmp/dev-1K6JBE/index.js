...
// .wrangler/tmp/bundle-Hz9489/checked-fetch.js
...
// .wrangler/tmp/bundle-Hz9489/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_process();
init_virtual_unenv_global_polyfill_performance();
...
// .wrangler/tmp/bundle-Hz9489/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_process();
init_virtual_unenv_global_polyfill_performance();
...
blogRouter.post("/create", async (c) => {
  const body = await c.req.json();
  const { success } = import_zod_inference_medium_blog2.createBlogInput.safeParse(body);
  if (!success) {
    c.status(400);
    return c.json({
      message: "Inputs not correct"
...
blogRouter.put("/update", async (c) => {
  const body = await c.req.json();
  const { success } = import_zod_inference_medium_blog2.updateBlogInput.safeParse(body);
  if (!success) {
    c.status(400);
    return c.json({
      message: "Inputs not correct"
